...
class Log {
 ...
 private:
  void Init(const char* file, int line) {
    ...
    char us[7];
    snprintf(us, sizeof(us), "%06ld", static_cast<long>(tvs.tv_usec));

    os_ << l[lvl_] << s << us << ' ' << pthread_self()
        << " Pid: " << getpid() << " Tid: " << absl::base_internal::GetTID()
        << " " << file << ':'
        << line << "] ";
  }
